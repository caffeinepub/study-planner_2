...
export function DailyProgressChart({ tasks }: DailyProgressChartProps) {
  const stats = useMemo(() => {
    const completed = tasks.filter((t) => t.isCompleted).length;
    const pending = tasks.length - completed;
    const totalMinutes = tasks.reduce((sum, task) => sum + parseDurationToMinutes(task.duration), 0);
    const totalTime = formatMinutesToDuration(totalMinutes);
    
    const completedPercent = tasks.length > 0 ? (completed / tasks.length) * 100 : 0;
    const pendingPercent = tasks.length > 0 ? (pending / tasks.length) * 100 : 0;
